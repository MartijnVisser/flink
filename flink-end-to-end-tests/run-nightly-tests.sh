...
function run_group_2 {
    ...
    if [[ ${PROFILE} != *"enable-adaptive-scheduler"* ]]; then # FLINK-21400
      run_test "File Sink end-to-end test" "$END_TO_END_DIR/test-scripts/test_file_sink.sh local FileSink" "skip_check_exceptions"
      run_test "File Sink s3 end-to-end test" "$END_TO_END_DIR/test-scripts/test_file_sink.sh s3 FileSink" "skip_check_exceptions"
